...
#print(get.get_amazon_data('pencil', 1))
...
allowable_items = [i.strip() for i in ['bicycle', ' backpack', ' umbrella', ' handbag', ' tie', ' suitcase', ' baseball bat', ' baseball glove', ' skateboard',
...
def refresh():
	try:
			for filez in os.listdir(upload):
				os.remove(os.path.join(upload, filez))
	except:
		pass
	return
...
def get_filenames():
	...
	import json
	detImages = [f for f in listdir('static/detections') if isfile(join('static/detections', f))]
